...
run_edna_model_full <- function(physeq_obj,
                                poisson_formula,
                                binomial_formula,
                                ref_otu = "OTU1005",
                                num_otus = 50,
  ...
  # --- Step 1: Filter taxa ---
  physeq_filtered <- filter_taxa(physeq_obj, function(x) sum(x) >= min_species_sum, prune = TRUE)

  # --- Step 2: Keep only desired sample type ---
  ...
  # --- Step 3: Parse Sample and Replicate ---
  sample_names_vec <- sample_names(physeq_bio)
  parsed <- tryCatch({
    as.data.frame(strcapture("^(.*)_r([0-9]+)$", sample_names_vec,
                             list(Sample = character(), Replicate = integer())))
  }, error = function(e) {
  ...
  # --- Step 7: Subset OTUs ---
  otu_levels <- levels(long_df$i)
  selected_otus <- head(otu_levels[otu_levels != ref_otu], num_otus - 1)
  selected_otus <- c(ref_otu, selected_otus)

  otu_subset <- long_df %>%
    filter(i %in% selected_otus) %>%
    droplevels()

  otu_subset$i <- factor(otu_subset$i, levels = c(ref_otu, setdiff(levels(otu_subset$i), ref_otu)))

  # --- Step 8: Filter treatment != "0" ---
  if (!treatment_var %in% colnames(otu_subset)) {
    stop(paste("Column", treatment_var, "not found in long_df"))
  }

  otu_subset <- otu_subset %>%
    filter(.data[[treatment_var]] != "0")

  otu_subset[[treatment_var]] <- droplevels(factor(otu_subset[[treatment_var]]))

  # --- Step 9: Check levels ---
  cat("✅ Treatment levels used:", paste(levels(otu_subset[[treatment_var]]), collapse = ", "), "\n")

  # --- Step 10: Initialize z_sim ---
  otu_subset$z_sim <- ifelse(otu_subset$y > 0, 1,
                             rbinom(nrow(otu_subset), 1, 0.5))
  ...
  for (iter in 1:num_iterations) {
    Q <- otu_subset[otu_subset$z_sim == 1, ]

    model_poisson <- glmmTMB(poisson_formula, family = poisson, data = Q)
    model_binomial <- glmmTMB(binomial_formula, family = binomial, data = otu_subset)

    lambda_i <- predict(model_poisson, type = "response", newdata = otu_subset)
    P_i <- predict(model_binomial, type = "response", newdata = otu_subset)

    prob_Z1_given_y0 <- P_i * exp(-lambda_i) / (P_i * exp(-lambda_i) + (1 - P_i))
    prob_Z1_given_y0 <- pmin(pmax(prob_Z1_given_y0, 1e-6), 1 - 1e-6)

    otu_subset$z_sim[otu_subset$y == 0] <-
      rbinom(sum(otu_subset$y == 0), 1, prob_Z1_given_y0[otu_subset$y == 0])
...
summarize_otu_level <- function(model_out, otu_subset) {
    # --- Predict occupancy (psi) ---
    psi_mat <- do.call(cbind, lapply(model_out$binomial_models, function(model) {
        predict(model, type = "response", newdata = otu_subset)
    }))
    
    # --- Predict abundance (lambda) ---
    lambda_mat <- do.call(cbind, lapply(model_out$poisson_models, function(model) {
        predict(model, type = "response", newdata = otu_subset)
    }))
...
predict_otu_treatment <- function(model_out, otu_subset) {
    # --- Predict occupancy (psi) ---
    psi_mat <- do.call(cbind, lapply(model_out$binomial_models, function(model) {
        predict(model, type = "response", newdata = otu_subset)
    }))
    ...
    return(otu_treatment_summary)
}

otu_summary <- summarize_otu_level(model_out, result$otu_subset)
otu_treatment_summary <- predict_otu_treatment(model_out, result$otu_subset)

head(otu_summary)
head(otu_treatment_summary)
