...
# Randomly sample 100 OTUs
set.seed(123)
selected_otus <- sample(taxa_names(physeq), 100)
physeq_subset <- prune_taxa(selected_otus, physeq)
...
p_detect_data <- out$summary %>%
  select(OTU, Site, treatment, 
         p_detect_mean, p_detect_lwr, p_detect_upr)

# Arrange OTUs by average detection probability for plotting
p_detect_data <- p_detect_data %>%
  group_by(OTU) %>%
...
ggplot(p_detect_data, aes(x = p_detect_mean, y = OTU)) +
  ...
  labs(
    title = "Caterpillar Plot of Detection Probability by OTU",
...
psi_data <- out$summary %>%
  select(OTU, Site, treatment,
         psi_mean, psi_lwr, psi_upr)

# Arrange OTUs by average psi_mean for consistent ordering on y-axis
psi_data <- psi_data %>%
  group_by(OTU) %>%
...
ggplot(psi_data, aes(x = psi_mean, y = OTU)) +
  ...
  labs(
    title = "Caterpillar Plot of Occupancy Probability by OTU",
    x = "Probability of Occupancy (ψ)",
    y = "OTU"
  ) +
  theme_minimal() +
  theme(
...
psi_data <- out$summary %>%
  ...
  mutate(OTU = factor(OTU, levels = unique(OTU)))  # Order OTUs

# Plot
ggplot(psi_data, aes(x = psi_mean, y = OTU, color = treatment)) +
  ...
  labs(
    title = "Probability of Occupancy (ψ) by OTU and Treatment",
...
Clustered Heatmap of Occupancy Probabilities (ψ)

library(dplyr)
library(tidyr)
library(pheatmap)

psi_summary <- out$summary %>%
  select(Site, OTU, treatment, psi_mean)

# Step 1: Summarise duplicated rows
psi_matrix <- psi_summary %>%
  group_by(Site, OTU) %>%
...
# Step 2: Replace NAs (optional but recommended)
