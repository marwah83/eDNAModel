...
#'
#' \deqn{
#'  A_{i,j,m} | Z_{i,m} \sim \mathrm{Bernoulli}(p_{i,j,m} \cdot Z_{i,m})
#' }
#'
#' \deqn{
#'  Y_{i,j,k,m} | A_{i,j,m} \sim \mathrm{Count}(\lambda_{i,j,k,m} \cdot A_{i,j,m}))
#' }
#'
#' where:
#' \itemize{
...
#'   \item Fit GLMM capture model for \(A | Z = 1\)
#'   \item Fit GLMM abundance model for \(Y | A = 1\)
