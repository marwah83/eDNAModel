...
# Step 1: Compute average ψ (occupancy) for each Site–OTU pair
otu_pred_df <- otu_subset %>%
  select(Site, OTU) %>%
  mutate(psi = rowMeans(psi_mat, na.rm = TRUE)) %>%
  group_by(Site, OTU) %>%
  summarise(psi = mean(psi, na.rm = TRUE), .groups = "drop")

# Step 2: Pivot to wide matrix: rows = Sites, columns = OTUs
psi_matrix <- otu_pred_df %>%
  pivot_wider(names_from = OTU, values_from = psi) %>%
...
pheatmap(
  mat = psi_matrix,
  cluster_rows = TRUE,     # Cluster Sites
  cluster_cols = TRUE,     # Cluster OTUs
