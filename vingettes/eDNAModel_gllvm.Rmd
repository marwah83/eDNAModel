...
# ------------------------------------------------------------
# 1. Automatically identify identifier columns
# ------------------------------------------------------------
...
site_mat <- as.matrix(
  out$mean_lv_sites[, c("LV1", "LV2"), drop = FALSE]
)

species_mat <- as.matrix(
  out$mean_lv_species[, c("LV1", "LV2"), drop = FALSE]
)

# ------------------------------------------------------------
# 3. Remove incomplete coordinates
# ------------------------------------------------------------

site_ok <- complete.cases(site_mat)
species_ok <- complete.cases(species_mat)

site_mat <- site_mat[site_ok, , drop = FALSE]
species_mat <- species_mat[species_ok, , drop = FALSE]

site_labels <- as.character(
  out$mean_lv_sites[[site_col]][site_ok]
)

species_labels <- as.character(
  out$mean_lv_species[[otu_col]][species_ok]
)
...
if (!is.finite(site_radius) || site_radius <= 0) {
  stop("Invalid site latent-variable coordinates.")
}

if (!is.finite(species_radius) || species_radius <= 0) {
  stop("Invalid OTU latent-variable coordinates.")
}
...
dim(scaled_sites_df)
dim(scaled_species_df)

head(scaled_sites_df)
head(scaled_species_df)
