...
filter_data_array <- function(data_array, min_species_sum = 30, save_path = "datanew_filtered.Rdata") {
  ...
  # 1. Remove species (rows) that are all zero
  ...
  data_array <- data_array[species_to_keep, , ]
  species_names_filtered <- dimnames(data_array)$Species
  message("✅ Species with all zeros removed. Remaining species: ", length(species_names_filtered))

  # 2. Remove sites (columns) that are all zero
  ...
  data_array <- data_array[, sites_to_keep, ]
  site_names_filtered <- dimnames(data_array)$Sites
  message("✅ Sites with all zeros removed. Remaining sites: ", length(site_names_filtered))

  # 3. Remove species (rows) with total counts less than specified threshold
  ...
  data_array_filtered <- data_array[species_to_keep_final, , ]
  message("✅ Species with total counts <", min_species_sum, " removed. Remaining species: ", dim(data_array_filtered)[1])
