...
# Summarise at OTU level (correct: eta is already p_detect)
p_detect_otu_summary <- p_detect_draws %>%
  group_by(OTU) %>%
  summarise(
    p_detect_mean = mean(eta, na.rm = TRUE),
    p_detect_lwr  = quantile(eta, 0.025, na.rm = TRUE),
    p_detect_upr  = quantile(eta, 0.975, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
ggplot(p_detect_otu_summary, aes(x = p_detect_mean, y = OTU)) +
  ...
  labs(
    ...
    x = "Detection probability (p_detect)",
    y = "OTU"
  ) +
  theme_minimal() +
  theme(
