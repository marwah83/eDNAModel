test_that("prepare_long_data returns correctly formatted output", {
    ...
    # Check physeq returned unchanged
    ...
    # -----------------------------
    expect_true(all(c("SampleRep", "OTU", "y", "site_field") %in% names(long_df)))
    ...
    # Site column should exist (not forced to factor here)
    expect_true("site_field" %in% names(long_df))
